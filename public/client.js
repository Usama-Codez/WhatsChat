                ...
                case 'message':
                    if (data.encrypted && myKeyPair?.privateKey) {
                        try {
                            ...
                            showNotification(`New message from ${data.sender}: ${decryptedMessage}`);
                        } catch (error) {
...
function handleFileSelect(event) {
    ...
    const vulnerableExtensions = ['.apk', '.exe', '.bat']; // Add more extensions as needed

    // Check for vulnerable file types
    const fileExtension = file.name.split('.').pop().toLowerCase();
    if (vulnerableExtensions.includes(`.${fileExtension}`)) {
